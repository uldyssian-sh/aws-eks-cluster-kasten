...
aws eks create-cluster --name test-cluster-permissions --version 1.29 --role-arn "${TEST_ROLE_ARN}" --resources-vpc-config subnetIds="${TEST_SUBNET}" --region us-west-2 --dry-run 2>&1 || echo "This shows what Success we'd get"